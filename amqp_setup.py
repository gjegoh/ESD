...
exchangename="mailchimp_topic"
exchangetype="topic"
channel.exchange_declare(exchange=exchangename, exchange_type=exchangetype, durable=True)
print("exchange set up")
...
############   approveTutor queue    #############
#delcare approveTutor queue
queue_name = 'approveTutor'
channel.queue_declare(queue=queue_name, durable=True)
    # 'durable' makes the queue survive broker restarts

#bind approveTutor queue
channel.queue_bind(exchange=exchangename, queue=queue_name, routing_key='*.approveTutor') 
print("approve tutor queue set up")
...
############   rejectTutor queue    #############
#delcare rejectTutor queue
queue_name = 'rejectTutor'
channel.queue_declare(queue=queue_name, durable=True)
    # 'durable' makes the queue survive broker restarts

#bind rejectTutor queue
channel.queue_bind(exchange=exchangename, queue=queue_name, routing_key='*.rejectTutor') 
print("reject tutor queue set up")
    # bind the queue to the exchange via the key
    # 'routing_key=#' => any routing_key would be matched
...
#bind paymentSuccessful queue
channel.queue_bind(exchange=exchangename, queue=queue_name, routing_key='*.paymentSuccessful') 
print("paymentSuccessful queue set up")
    # bind the queue to the exchange via the key
