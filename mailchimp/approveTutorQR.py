...
monitorBindingKey='*.approveTutor'

def receiveApprovedTutor():
    print("running receiveNewStudent")
    amqp.check_setup()
    queue_name = "approveTutor"
...
def callback(channel, method, properties, body): # required signature for the callback; no return
    print("\nReceived an tutor approval by " + __file__)
    processApproveTutor(body)
    print() # print a new line feed

def processApproveTutor(body):
    print("Printing the tutor approval message:")
    try:
    ...
    #create contact for approved tutors 
    if apiKeyStatus == 200:
        email = message['email']
        ...
        if addNewStudentStatus == 200:
            print("add approved tutors into approve segment:")
            segment_id, createSegmentStatus, createSegmentResponse = mailchimpFunctions.addMemberIntoSegment("approve", email)
            print(createSegmentStatus)
            print(createSegmentResponse)
            print(segment_id)
            #create campaign
            if createSegmentStatus == 200:
                campaign_name = "Approval of Tutor Application"
                ...
                if createCampaignStatus == 200:
                    ...
                    addContentStatus, addContentResponse = mailchimpFunctions.addContent(campaign_id, "<body><h1>We have approved your tutor application</h1><p>You can now apply for tuition classes.</p></body>")
...
if __name__ == "__main__":  # execute this program only if it is run as a script (not by 'import')    
    ...
    print(": monitoring routing key '{}' in exchange '{}' ...".format(monitorBindingKey, amqp.exchangename))
    receiveApprovedTutor()